...
describe('old contract interactions', () => {
    const myComp = setup();
...
describe('new contract interactions', () => {
    const myComp = setup();
    
    it('erc20 fetch symbol and decimal #1', async () => {
        let res = await myComp.tokenGetSymbolAndDecimal('0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48')
        expect(res).to.eql(['USDC', '6'])
    })

    it('erc20 fetch symbol and decimal #2', async () => {
        let res = await myComp.tokenGetSymbolAndDecimal('0x514910771af9ca656af840dff83e8264ecf986ca')
        expect(res).to.eql(['LINK', '18'])
    })

    it('check is token ERC721 #1', async () => {
        let res = await myComp.isERC721('0x9C8fF314C9Bc7F6e59A9d9225Fb22946427eDC03')
        expect(res).to.eql(true)
    })

    it('check is token ERC721 #2', async () => {
        let res = await myComp.isERC721('0xdac17f958d2ee523a2206206994597c13d831ec7')
        expect(res).to.eql(false)
    })

    it('erc721 fetch symbol and decimal', async () => {
        let res = await myComp.tokenGetSymbolAndDecimal('0x9C8fF314C9Bc7F6e59A9d9225Fb22946427eDC03')
        expect(res).to.eql(['NOUN', '0'])
    })

    
    it('ERC20 check balance', async () => {
        let res = await myComp.checkWalletTokenBalance('0xedcc867bc8b5febd0459af17a6f134f41f422f0c', '0x9F6F91078A5072A8B54695DAfA2374Ab3cCd603b', 18)
        expect(res).greaterThan(1)
    })

    it('ERC721 check balance', async () => {
        let res = await myComp.checkWalletTokenBalance('0xAe7f458667f1B30746354aBC3157907d9F6FD15E', '0x9C8fF314C9Bc7F6e59A9d9225Fb22946427eDC03', 0)
